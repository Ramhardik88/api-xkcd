...
class SimpleEmailService
{
     ...
     */
	protected $requestSignatureVersion;
     ...
     */
	public function __construct($accessKey = null, $secretKey = null, $host = self::AWS_AP_SOUTH_1, $trigger_errors = true, $requestSignatureVersion = self::REQUEST_SIGNATURE_V4) {
		if ($accessKey !== null && $secretKey !== null) {
			$this->setAuth($accessKey, $secretKey);
		}
		$this->host = $host;
		$this->trigger_errors = $trigger_errors;
		$this->requestSignatureVersion = $requestSignatureVersion;
	}
    ...
    public function getRequestSignatureVersion() {
	    return $this->requestSignatureVersion;
    }
	...
	*/
	public function setBulkMode($enable = true) {
		$this->bulk_sending_mode = (bool)$enable;
		return $this;
	}
	...
	*/
	public function sendEmail($sesMessage, $use_raw_request = false , $trigger_error = null) {
		...
		if($ses_response->error !== false) {
			if (($this->trigger_errors && ($trigger_error !== false)) || $trigger_error === true) {
	...
	*/
	...
	public function getRequestHandler($verb) {
		if (empty($this->ses_request)) {
			$this->ses_request = new SimpleEmailServiceRequest($this, $verb);
		} else {
			$this->ses_request->setVerb($verb);
		}

		return $this->ses_request;
	}
